...
def parse_gurobipy_optimizer_results(problem: Problem, evaluator: GurobipyEvaluator) -> SolverResults:
    ...
    variable_values = {var.symbol: results[var.symbol] for var in problem.variables} if len(results) > 1 else {}
    objective_values = {obj.symbol: results[obj.symbol] for obj in problem.objectives} if len(results) > 1 else {}
    constraint_values = {con.symbol: results[con.symbol] for con in problem.constraints} if len(results) > 1 else {}
