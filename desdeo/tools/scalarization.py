...
def add_group_asf(
    ...
    func = f"{symbol}_max_ + {rho} * ({aug_exprs})"
    ...
    scalarization_function = ScalarizationFunction(
        ...
        sub_constraints=[SubConstraint(
            name=f"{symbol} MAX subconstraint",
            symbol=f"{symbol}_max_",
            func=max_terms,
            cons_type=SubConstraintTypeEnum.MAX,
        )],
...
def add_group_stom_sf(
    ...
    func = f"{symbol}_max_ + {rho}*" + f"({aug_exprs})"
    scalarization = ScalarizationFunction(
